...
case "$file_path" in
  */docs/plans/*.md)
    ...
    cat <<'MSG'
NOTE: You are writing a plan document directly. These should normally be
...
MSG
  ...
  */docs/tasks/*.md)
    ...
    cat <<'MSG'
NOTE: You are writing a task document directly. These should normally be
generated using the generate-task skill (invoke via Skill tool or /kenspc-task)
which reads actual code for correct decomposition and self-reviews via review agent.
If you have already invoked the skill, ignore this message.
MSG
    exit 0
    ;;
