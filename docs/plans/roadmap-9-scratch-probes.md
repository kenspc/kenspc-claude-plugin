...
| M5 | A new guard? | No. The two byte-identical carriers are already guarded (`check-review-agent-drift.sh`, `check-run-contract.sh`); the code-fixer and regression-verifier sentences are not cross-file anchors. The falsifiable check is the release checklist's run-directory check (Step 3.3). |
...
| A linter or build tool walks into the run directory (ESLint's flat config lints dot-directories by default) | Medium | No name keeps a source probe out of a linter's pattern, so the naming rule does not extend to it; `regression-verifier` runs build, test, and lint unmodified and names the scratch files in the FAIL (C5). Tracked as a roadmap item (Step 3.4). |
...
- C4 — code-fixer's reply is also enumerated outside `code-fixer.md`, and each enumeration gains "and the scratch-pollution note, when there is one". The places are:
  - the "Its reply carries …" paragraph and the Schema F `## Fixes` placeholder in `task-review/SKILL.md`, and the same paragraph and the Schema G `## Fixes` placeholder in `task-implement/SKILL.md` (Step 2.1). These four spots lie outside every canonical block.
  - the INPUTS description of `schema-b.md` in `regression-verifier.md` (Step 1.3);
  - the first bullet of the plugin README's Run directory section (Step 3.1).

  No Next steps bullet is added. When the note exists, `regression-verifier`'s unmodified run records the same files as FAIL, and the FAIL verdict already carries them with a suggested action.
- C5 — Lint and build still walk into `.kenspc/`. ESLint's flat config ignores only `node_modules` and `.git` by default ("In flat config, dotfiles … are no longer ignored by default", ESLint configuration migration guide), so a runner-safe `probe.mts` can still fail `eslint .`.
