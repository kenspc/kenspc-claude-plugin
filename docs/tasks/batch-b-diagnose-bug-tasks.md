...
**Implementation notes:**
- Decisions: both messages open with "generated by a kenspc skill:" and
  close with "If you have already invoked one of them, ignore this message."
  — one closing sentence for the two branches, as the task asks, and plural
  because each branch now names two skills.
- Changes/tradeoffs: none. The four probes behaved as required: the tasks
  and briefs messages each name both skills and both commands, the
  `_template.md` probe printed nothing and exited 0, and the plan message is
  byte-identical to its output before the edit (`cmp` of the two captures).
